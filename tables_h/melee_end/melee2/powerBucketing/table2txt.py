...
try:
    ...
    if tableType == "qtable":
        if len(sys.argv) < 3:
            print("Error: missing arguments", len(sys.argv))
            exit(1)
        else:
        ...
        for key, value in t_table.items():
            if key != "TrialsData":
                strTransitions = "for state " +  key + ": action count = " + str(value[1]) + "\n"
                strTransitions += value[0].to_string() + "\n\n"
                f.write(strTransitions)
